...
export default function SuratMasukModal({ data, onClose, allOptions, userInfo }) {
    const [formData, setFormData] = useState({ 
        nomorSurat: '', tanggalSurat: '', pengirim: '', perihal: '', 
        sifatSurat: 'Biasa', jenisSurat: '', waktuPelaksanaan: '', tempatPelaksanaan: '', ...data 
    });
    ...
    useEffect(() => {
        if (data?.tanggalSurat?.toDate) {
            setFormData(prev => ({...prev, tanggalSurat: data.tanggalSurat.toDate().toISOString().split('T')[0]}));
        }
        if (data?.waktuPelaksanaan?.toDate) {
            const d = data.waktuPelaksanaan.toDate();
            d.setMinutes(d.getMinutes() - d.getTimezoneOffset());
            setFormData(prev => ({...prev, waktuPelaksanaan: d.toISOString().slice(0,16)}));
        }
    }, [data]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            notifier.show('Data berhasil disimpan!', 'success');
            onClose();
        } catch (error) {
            console.error(error);
    ...
    const { tujuan = [], jenisSurat = [] } = allOptions;
